...
export const Grid = React.memo((props: GridProps) => {
  ...
  const gridColumns = React.useMemo(() => {
    return columns
      ...
      .map((col) => {
        ...
        const filtered = filtering?.conditions?.find(
          (f) => f.attributeName === col.name,
        );
        return {
          key: col.name,
  ...
  const onRenderRow: IDetailsListProps["onRenderRow"] = (props) => {
    ...
    if (props?.item) {
      if (
        highlightColor &&
        highlightValue &&
        props.item.getValue("HighlightIndicator") === highlightValue
      ) {
  ...
  const handleOpenEdit = React.useCallback(() => {
    ...
    setEditingData({
      ...
      learningStatus:
        (record.getValue("ksvc_opt_learningstatus") as number) || undefined,
  ...
  const handleDelete = React.useCallback(async () => {
    if (
      selectedId &&
      confirm("Bạn có chắc chắn muốn xoá bản ghi sinh viên này?")
    ) {
      setIsLoading(true);
  ...
  return (
    <Stack verticalFill grow style={rootContainerStyle}>
      ...
      <Stack.Item
        grow
        style={{ position: "relative", backgroundColor: "white", zIndex: 0 }}
      >
        {!itemsLoading &&
          !isComponentLoading &&
          items &&
          items.length === 0 && (
            <Stack grow horizontalAlign="center" className="noRecords">
              ...
              <Text variant="large">
                {resources.getString("Label_NoRecords")}
              </Text>
            </Stack>
          )}
      ...
      <Stack.Item>
        ...
        >
          ...
          <Stack.Item grow align="center" style={{ textAlign: "center" }}>
            {!isFullScreen && (
              <Link onClick={onFullScreen}>
                {resources.getString("Label_ShowFullScreen")}
              </Link>
            )}
          </Stack.Item>
          <IconButton
